...
BattleshipGame.prototype.shoot = function(position) {
  //opponenet is opposite of current player
...
BattleshipGame.prototype.getGameState = function(player, gridOwner) {
  return {
    ...
    playerGridSide: player === gridOwner ? 0 : 1,       // which client grid to update (0 = own, 1 = opponent)
    grid: this.getGrid(gridOwner, player !== gridOwner)  // hide unsunk ships if this is not own grid
    //grid contain the array[10*10]shots for each player and how many ship opponent sunk down and you has how many ships in total(const)
  };
};
...
BattleshipGame.prototype.getGrid = function(player, hideShips) {
  return {
    ...
    //if the hideShips( player is not grid owner is TRUE) return how many ship player suck as Array
    //if the hideShips(play is the grid owner) return all the ships he has
