...
Player.prototype.createRandomShips = function() {
  ...
  for(shipIndex = 0; shipIndex < Settings.ships.length; shipIndex++) {
    ...
    this.placeShipRandom(ship, shipIndex)
...
Player.prototype.placeShipRandom = function(ship, shipIndex) {
  var placedShipFlag = false;
  var i, gridIndex, xMax, yMax;

  while(!placedShipFlag){ //true if not overlap and adjacent
    ship.horizontal = Math.random() < 0.5; //true: horziontal, false vertical
    //depend on the ship length, what's the max x can it be
    xMax = ship.horizontal ? Settings.gridCols - ship.size : Settings.gridCols - 1;
    //depend on the ship length, what's the max y can it be
    yMax = ship.horizontal ? Settings.gridRows - 1 : Settings.gridRows - ship.size;
